...
export class HealthScoreService {
  calculateHealthScore(metrics: SystemMetrics, factors: Partial<HealthFactors> = {}): HealthResult {
    const defaults: HealthFactors = {
      cpuUsage: metrics.cpu.usage,
      memoryUsage: metrics.memory.percentage,
      diskUsage: metrics.disk.total > 0 ? (metrics.disk.used / metrics.disk.total) * 100 : 0,
    ...
    const securityScore = healthFactors.securityIssues === 0 ? 100 : Math.max(0, 100 - healthFactors.securityIssues * 10);
