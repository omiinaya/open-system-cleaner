...
export class TurboModeService {
  ...
  async enable(): Promise<TurboModeResult> {
    ...
    try {
      ...
      if (serviceResult.stopped > 0) {
        optimizations.push(`${serviceResult.stopped} unnecessary services stopped`);
      }
  ...
  async disable(): Promise<boolean> {
    try {
      ...
      if (process.platform === 'win32') {
        await execAsync('reg add "HKCU\\Control Panel\\Desktop" /v UserPreferencesMask /t REG_BINARY /d 9012008010000000 /f');
      }
  ...
  private async clearSystemCaches(): Promise<number> {
    ...
    try {
      if (process.platform === 'win32') {
        ...
        await execAsync('rmdir /s /q C:\\Windows\\SoftwareDistribution\\Download');
  ...
  private async optimizeServices(): Promise<{ stopped: number; services: string[] }> {
    const stopped = 0;
  ...
  private async disableVisualEffects(): Promise<void> {
    try {
      if (process.platform === 'win32') {
        ...
        await execAsync('reg add "HKCU\\Control Panel\\Desktop" /v UserPreferencesMask /t REG_BINARY /d 9032078010000000 /f');
        await execAsync('reg add "HKCU\\Control Panel\\Desktop\\WindowMetrics" /v MinAnimate /t REG_SZ /d 0 /f');
        
        // Set performance options
        await execAsync('reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 2 /f');
      }
    } catch (error) {
  ...
  private async setHighPriority(): Promise<void> {
    try {
      ...
      if (process.platform === 'win32') {
        await execAsync(`wmic process where "ProcessId=${process.pid}" CALL setpriority 128`);
      } else {
  ...
  private async setNormalPriority(): Promise<void> {
    try {
      if (process.platform === 'win32') {
        await execAsync(`wmic process where "ProcessId=${process.pid}" CALL setpriority 32`);
      } else {
        // Reset nice value
