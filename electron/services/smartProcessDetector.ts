...
export class SmartProcessDetector {
  private processHistory = new Map<number, {
    cpuReadings: number[];
   ...
   */
  async getProcessContext(pid: number, processName: string): Promise<ProcessContext> {
    ...
    try {
      ...
      context.hasUserInput = Date.now() - context.lastActivityTime < this.ACTIVITY_THRESHOLD;
   ...
   */
  async shouldTerminateProcess(pid: number, processName: string): Promise<{
    ...
    if (context.memoryTrend === 'increasing') {
      await auditLogger.log('process_memory_increasing', 'smartProcessDetector', 'warning', {
        pid,
        name: processName,
      });
    ...
    const avgCpu = context.cpuHistory.reduce((a, b) => a + b, 0) / context.cpuHistory.length;
  ...
  private async getWindowTitle(pid: number): Promise<string | undefined> {
    if (process.platform !== 'win32') {
      ...
      try {
        const { stdout } = await execAsync(`xdotool getwindowname $(xdotool search --pid ${pid} | head -1) 2>/dev/null || echo ""`);
  ...
  private async isForegroundProcess(pid: number): Promise<boolean> {
    if (process.platform !== 'win32') {
      ...
      try {
        const { stdout } = await execAsync('xdotool getactivewindow getwindowpid 2>/dev/null || echo "0"');
  ...
  private async getLastActivity(pid: number): Promise<number> {
    ...
    try {
      ...
      const { stdout } = await execAsync(`ps -p ${pid} -o lstart= 2>/dev/null || echo ""`);
      if (stdout.trim()) {
   ...
   */
  private async hasUnsavedWork(pid: number, windowTitle?: string): Promise<boolean> {
    // Check window title for indicators of unsaved work
    if (windowTitle) {
      const unsavedIndicators = ['*', '●', 'modified', 'edited', 'unsaved'];
      if (unsavedIndicators.some(indicator => windowTitle.includes(indicator))) {
        return true;
      }
    }
  ...
  private async updateProcessHistory(pid: number): Promise<void> {
    try {
      ...
      if (Date.now() - existing.lastCheck > 300000) { // 5 minutes
   ...
   */
  private calculateMemoryTrend(readings: number[]): 'increasing' | 'decreasing' | 'stable' {
  ...
  cleanupOldHistory(): void {
    ...
    for (const [pid, history] of this.processHistory.entries()) {
      if (now - history.lastCheck > 600000) { // 10 minutes
        this.processHistory.delete(pid);
      }
    }
  }
