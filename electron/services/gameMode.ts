...
export class GameModeService {
  ...
  async enable(gameName?: string): Promise<GameModeResult> {
    ...
    try {
      ...
      if (processesSuspended > 0) {
        optimizations.push(
          `${processesSuspended} background processes suspended`,
        );
      }
  ...
  private async suspendBackgroundProcesses(): Promise<{
    processes: string[];
    count: number;
  }> {
  ...
  private async setGamePriority(gameName: string): Promise<void> {
    try {
      ...
      if (process.platform === "win32") {
        ...
        await execAsync(
          `wmic process where "name='${gameInfo.executable}'" CALL setpriority 256`,
        );
      } else {
        // On Linux, use nice
        const { stdout } = await execAsync(
          `pgrep -f "${gameInfo.processName}"`,
        );
        const pids = stdout.trim().split("\n");
    ...
    try {
      ...
      for (const proc of processes) {
        try {
          if (process.platform === "win32") {
            await execAsync(
              `wmic process where "ProcessId=${proc.pid}" CALL setpriority 32`,
            );
          } else {
