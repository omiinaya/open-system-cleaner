...
export class SystemMetricsService {
  ...
  async performHealthCheck(): Promise<{
    ...
    try {
      ...
      if (freeGB < CONSTANTS.LOW_DISK_SPACE_GB) {
        issues.push(`Critical: Less than ${CONSTANTS.LOW_DISK_SPACE_GB}GB disk space remaining (${freeGB.toFixed(2)}GB free)`);
      }
      
      // Check memory
      if (metrics.memory.percentage > CONSTANTS.HIGH_MEMORY_PERCENTAGE) {
        issues.push(`Warning: Memory usage critical (${metrics.memory.percentage.toFixed(1)}%)`);
      }
      ...
      await auditLogger.log('health_check', 'systemMetrics', issues.length === 0 ? 'success' : 'warning', {
  ...
  startMonitoring(callback: (metrics: SystemMetrics) => void, _interval = 5000) {
