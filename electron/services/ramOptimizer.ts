...
export class RAMOptimizerService {
    ...
    try {
      ...
      return processes.list
        ...
        .map((p) => ({
          ...
          memoryPercent: p.memRss
            ? (p.memRss / (os.totalmem() / 1024)) * 100
            : 0,
  ...
  async optimize(
    ...
    // Terminate processes
    for (const proc of processesToTerminate.slice(0, 10)) {
