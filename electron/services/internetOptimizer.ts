...
export class InternetOptimizerService {
  ...
  async optimize(options: {
    ...
    const { optimizeDNS = true, optimizeTCP = true, optimizeBrowser = true } = options;
  ...
  private async optimizeDNS(): Promise<boolean> {
    try {
      ...
      if (process.platform === 'win32') {
        ...
        await execAsync(`netsh interface ip set dns "Wi-Fi" static ${fastestDNS.primary}`);
        await execAsync(`netsh interface ip add dns "Wi-Fi" ${fastestDNS.secondary} index=2`);
      } else if (process.platform === 'linux') {
        // Update resolv.conf (requires sudo)
        // Note: This is temporary and will be reset on reboot
        await execAsync(`echo "nameserver ${fastestDNS.primary}" | sudo tee /etc/resolv.conf`);
        await execAsync(`echo "nameserver ${fastestDNS.secondary}" | sudo tee -a /etc/resolv.conf`);
      } else if (process.platform === 'darwin') {
        // Set DNS on macOS using networksetup
        await execAsync(`networksetup -setdnsservers Wi-Fi ${fastestDNS.primary} ${fastestDNS.secondary}`);
      }
  ...
  private async findFastestDNS(): Promise<{ name: string; primary: string; secondary: string }> {
