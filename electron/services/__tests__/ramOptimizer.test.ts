...
describe("RAMOptimizerService", () => {
    ...
    it("should sort processes by memory usage (highest first)", async () => {
      ...
      if (processes.length > 1) {
        for (let i = 0; i < processes.length - 1; i++) {
          expect(processes[i].memory).toBeGreaterThanOrEqual(
            processes[i + 1].memory,
          );
        }
      }
    });
  ...
  describe("optimize", () => {
    ...
    it("should respect aggressive mode option", async () => {
      const normalResult = await ramOptimizerService.optimize({
        aggressive: false,
      });
      const aggressiveResult = await ramOptimizerService.optimize({
        aggressive: true,
      });
    ...
    it("should respect includeUserApps option", async () => {
      const systemOnlyResult = await ramOptimizerService.optimize({
        includeUserApps: false,
      });
      const includeUserResult = await ramOptimizerService.optimize({
  ...
  describe("getMemoryInfo", () => {
    ...
    it("should return consistent memory values", async () => {
      ...
      expect(Math.abs(memoryInfo.total - calculatedTotal)).toBeLessThan(
        memoryInfo.total * 0.1,
      );
    });
  ...
  describe("memory calculation", () => {
    ...
    it("should calculate memory percentage correctly", async () => {
      ...
      if (processes.length > 0) {
        ...
        expect(Math.abs(process.memoryPercent - expectedPercent)).toBeLessThan(
          1,
        );
      }
    });
