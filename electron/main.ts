...
ipcMain.handle(
  ...
  async (_event, timeRange: "1h" | "24h" | "7d" | "30d") => {
...
ipcMain.handle(
  "cleanup:cleanJunkFilesByCategory",
  async (
    _event,
...
ipcMain.handle(
  "privacy:cleanBrowserData",
  async (_event, itemPaths: string[]) => {
...
ipcMain.handle("optimization:boostPerformance", async () => {
  ...
  return {
    ...
    optimizationsApplied:
      ramResult.processesTerminated + (junkResult.success ? 1 : 0),
...
ipcMain.handle(
  "optimization:disableStartupProgram",
  async (_event, programId: string) => {
    return await startupManagerService.disableStartupProgram(programId);
  },
);

ipcMain.handle(
  "optimization:enableStartupProgram",
  async (_event, programId: string) => {
...
ipcMain.handle(
  "security:enableSystemHardeningMonitor",
...
ipcMain.handle(
  "streaming:findDuplicates",
  async (_event, { paths, options }) => {
    try {
      const duplicates = await streamingFileScanner.findDuplicatesStream(
        paths,
        options,
      );
...
ipcMain.handle("workers:hashFiles", async (_event, filePaths: string[]) => {
  try {
    ...
    console.log(
      `Worker threads hashed ${filePaths.length} files in ${duration}ms`,
    );
