  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-text-secondary">
                Boost gaming performance, manage processes, and activate turbo
                mode for maximum speed.
              </p>
            </div>
            <div className="flex-shrink-0">
              <Button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          value={turboMode ? "Active" : "Inactive"}
          label="Turbo Mode"
          icon={Rocket}
          iconColor={turboMode ? "success" : "primary"}
          size="sm"
        />
        <StatCard
          value={gameMode ? "On" : "Off"}
          label="Game Mode"
          icon={Gamepad2}
          iconColor={gameMode ? "success" : "primary"}
          size="sm"
        />
        <StatCard
          value={`${optimizedCount}/${games.length}`}
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-success-500/10 flex items-center justify-center">
                <Gamepad2 className="w-5 h-5 text-success-500" />
              </div>
              <div>
            ...
            <div className="space-y-2">
              {games.map((game) => (
                ...
                >
                  ...
                  <Badge
                    variant={game.optimized ? "success" : "default"}
                    size="sm"
                  >
                    {game.optimized ? "Optimized" : "Not Optimized"}
                  </Badge>
                </div>
              ))}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {processes
                .slice(0, showProcesses ? processes.length : 4)
                .map((process) => (
                  <div
                    key={process.id}
                    className="flex items-center justify-between p-3 bg-bg-tertiary rounded-lg"
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-1 text-sm text-text-secondary">
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-success-500" /> Disables
                  unnecessary visual effects
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-success-500" /> Prioritizes CPU
                  for active applications
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-success-500" /> Frees up RAM
                  automatically
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-success-500" /> Disables
