...
const OptimizeModule: React.FC = () => {
  ...
  const highImpactCount = startupPrograms.filter(
    (p) => p.enabled && p.impact === "high",
  ).length;
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-text-secondary">
                Optimize startup programs, free up RAM, boost internet speed,
      ...
      {isOptimizing && (
        <ScanProgress
          ...
          itemsScanned={Math.floor((optimizationProgress / 100) * 100)}
          totalItems={100}
          issuesFound={0}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          label="RAM Usage"
          icon={Cpu}
          iconColor={ramUsage > 80 ? "warning" : "success"}
          size="sm"
        />
        <StatCard
          value="45ms"
        ...
        />
        <StatCard
          value={`${drives.filter((d) => d.fragmentation > 20).length}`}
          label="Need Defrag"
          icon={HardDrive}
          iconColor={
            drives.some((d) => d.fragmentation > 20) ? "warning" : "success"
          }
          size="sm"
        />
      </div>
          ...
          <CardHeader
            action={
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setShowStartupDetails(!showStartupDetails)}
              >
                {showStartupDetails ? "Hide" : "Manage"}
              </Button>
            }
          >
          ...
          <CardContent>
            {highImpactCount > 0 && (
              <div className="mb-4 p-3 bg-warning-500/10 border border-warning-200 rounded-lg flex items-start gap-2">
                ...
                <p className="text-sm text-text-secondary">
                  {highImpactCount} high-impact programs slowing startup.
                </p>
              </div>
            )}
            {showStartupDetails ? (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <Switch
                        ...
                        onCheckedChange={() => toggleStartupProgram(program.id)}
                        size="sm"
                      />
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between p-4 bg-bg-tertiary rounded-lg">
                ...
                <Badge variant={highImpactCount > 2 ? "warning" : "success"}>
                  {highImpactCount > 2 ? "Slow" : "Fast"}
                </Badge>
              </div>
            )}
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span
                className={cn(
                  "text-lg font-bold",
                  ramUsage > 80 ? "text-danger-500" : "text-text-primary",
                )}
              >
                {ramUsage}%
              </span>
            ...
            <Progress
              ...
              max={100}
              color={
                ramUsage > 80 ? "danger" : ramUsage > 60 ? "warning" : "success"
              }
              size="lg"
              showLabel
            />
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="p-2 bg-bg-tertiary rounded-lg">
                <p className="text-lg font-semibold text-text-primary">16 GB</p>
                <p className="text-xs text-text-secondary">Total</p>
              </div>
              <div className="p-2 bg-bg-tertiary rounded-lg">
                <p className="text-lg font-semibold text-text-primary">
                  {((16 * ramUsage) / 100).toFixed(1)} GB
                </p>
                <p className="text-xs text-text-secondary">Used</p>
              </div>
              <div className="p-2 bg-bg-tertiary rounded-lg">
                <p className="text-lg font-semibold text-text-primary">
                  {((16 * (100 - ramUsage)) / 100).toFixed(1)} GB
                </p>
            ...
            {[
              {
                ...
                desc: "Use faster DNS servers",
              },
              {
                key: "tcpOptimization",
                label: "TCP Optimization",
                desc: "Optimize TCP settings",
              },
              {
                key: "browserOptimization",
                label: "Browser Optimization",
                desc: "Optimize browser settings",
              },
            ].map((item) => (
              <div
                key={item.key}
              ...
              >
                ...
                <Switch
                  checked={
                    internetSettings[item.key as keyof typeof internetSettings]
                  }
              ...
              <div key={drive.letter} className="p-4 bg-bg-tertiary rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant={drive.type === "SSD" ? "success" : "default"}
                      size="sm"
                    >
                      {drive.type}
                    </Badge>
                  ...
                  <span
                    className={cn(
                      "text-sm font-medium",
                      drive.fragmentation > 20
                        ? "text-warning-500"
                        : "text-success-500",
                    )}
                  >
                    {drive.fragmentation}% fragmented
                  </span>
                </div>
                <Progress
                  value={drive.used}
                  max={drive.total}
                  color={drive.fragmentation > 20 ? "warning" : "success"}
                  size="md"
                  showLabel
                />
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="text-text-tertiary">
                    {drive.free} GB free of {drive.total} GB
                  </span>
                  {drive.type === "HDD" && (
                    <Button
                      variant="primary"
                      size="sm"
                      onClick={() => handleDefrag(drive.letter)}
                      loading={defraggingDrive === drive.letter}
                    >
                      {defraggingDrive === drive.letter
                        ? "Defragging..."
                        : "Defrag"}
                    </Button>
                  )}
